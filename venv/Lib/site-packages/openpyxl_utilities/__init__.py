...
def col_to_num(col):
	'''Given an Excel column name (like  "A", "C" or "BA"), the string of a number of a column or the number of a
	column, will return the corresponding number.'''
...
def num_to_col(n):
	'''Given an Excel column name (like  "A", "C" or "BA"), the string of a number of a column or the number of a
	column, will return the corresponding column name.'''
	if isinstance(n, int):
		s = ""
		while n > 0:
...
def clear_sheet(ws):
	'''Will clear all the values of the given worksheet (if any).'''
	for row in ws.rows:
		ws._current_row -= 1
		for cell in row:
			cell.value = None
...
def copy_with_name(source_ws, name):
	'''Will generate a copy of a given worksheet and rename said copy to the given name. Both the source and the copy
	will share the same workbook and that workbook will be updated to have the new worksheet.'''
	wb = source_ws.parent
