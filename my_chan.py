...
#     print baohan_columns
    dfbao = pd.DataFrame(np.zeros(len(k_data.index)*len(baohan_columns)).reshape(len(k_data.index),len(baohan_columns)), index = k_data.index,columns=baohan_columns)    if not in_chan else k_data
#     print dfbao
    # stdt = k_data.index[0]
    ...
    for i, dt in enumerate(k_data.index):
        ...
        # 非包含情况
        if (k_data['high'][dt]>dfbao['high'][dt_bf] and k_data['low'][dt]>dfbao['low'][dt_bf])        or (k_data['high'][dt]<dfbao['high'][dt_bf] and k_data['low'][dt]<dfbao['low'][dt_bf]):
            if in_chan:
                dt_bf = dt
                continue
        ...
        dt_bf = set_baohan_value(dt_bf,                                  [enddate, dfbao['open'][dt_bf], k_data['close'][dt],                                  max(k_data['high'][dt], dfbao['high'][dt_bf]),                                  min(k_data['low'][dt], dfbao['low'][dt_bf]),                                  k_data['volume'][dt]+dfbao['volume'][dt_bf],                                  k_data['money'][dt]+dfbao['money'][dt_bf]])
        if in_chan:
...
def parse2ChanFen(chanK):
    ...
    def appendFen(ft, fidx):
        ...
        if fenType_bf == ft:
             ...
             fidx = fenIdx_bf if (ft==1 and chanK['high'][fenIdx_bf]>chanK['high'][fidx])             or (ft==-1 and chanK['low'][fenIdx_bf]<chanK['low'][fidx]) else fidx
        fenTypes.append(ft)
        fenIdx.append(fidx)
...
#         print '破坏结果', _break, fenIdx[_breaki_k], fenIdx[_breakj_k]
    ...
    def judgeBreak(fenType, bf, af):
        return (fenType==-1 and chanK['low'][af]<chanK['low'][bf])            or (fenType==1 and chanK['high'][af]>chanK['high'][bf])
...
#                             print '尾分型破坏并连接上一点, 移除分型:%d，旧分型:%d, 新的分型:%d'%(fa_, fb_,fenIdx[breakBj])
                ...
                toConBfIdx = breakBj
            return toConBfIdx,  -1, has_bi_error# -1：break 1:continue 0:不执行
        if breakBj>0: 
#                         breakBj = breakBj1 if breakBj==-1 or judgeBreak(fenTypes[i-1], fenIdx[breakBj], fenIdx[breakBj1]) else breakBj
            if j+2>=len(fenIdx) and len(biIdx)>1 and            judgeBreak(fenTypes[i-1], biIdx[len(biIdx)-2], fenIdx[breakBj]):
                fa_=biIdx.pop()
                fb_=biIdx.pop()
#                             print '尾分型破坏并连接上一点, 移除分型:%d，旧分型:%d, 新的分型:%d'%(fa_, fb_,fenIdx[breakBj])
                biIdx.append(fenIdx[breakBj])
                ...
                return toConBfIdx,  -1, has_bi_error # -1：break 1:continue 0:不执行
            return toConBfIdx,  1, has_bi_error # -1：break 1:continue 0:不执行
        return toConBfIdx,  0, has_bi_error # -1：break 1:continue 0:不执行
...
#                 print flag, breakBi 
                if flag: 
                    toConBfIdx, _bcn, has_bi_error = reAssignBi(biIdx, breakBi, breakBj, i, j)
                    if _bcn==-1:break
                    if _bcn==1:continue
                if len(biIdx)==0: 
                    biIdx.append(kidx)
...
#                 print toConBfIdx
    ...
    return biIdx, frsBiType, has_bi_error
#最终线段生成
#1、遍历相对高低点，判断线段破坏，
...
#             print '开始逆向破坏：逆向分型，逆向点，线段点', xfenTypesn[k], nxIdx, xdIdxn[k]
            if (xfenTypesn[k]==-1 and chanK['low'][xdIdxn[k]]<chanK['low'][nxIdx])            or (xfenTypesn[k]==1 and chanK['high'][xdIdxn[k]]>chanK['high'][nxIdx]):
...
#         print '逆向破坏', nxIdx, xdIdxn, xfenTypesn
    ...
    def judgeBreak(fenType, afPrice, idx, chanK):
        return (fenType==-1 and afPrice<chanK['low'][idx])     or (fenType==1 and afPrice>chanK['high'][idx])
    for i, idx in enumerate(biIdx):
        if afIdx < 0 :break # 线段破坏以后没有合适线段
        fenType = 1 if (frsBiType==1 and i%2==1) or (frsBiType==-1 and i%2==0)         else -1
#         print '开始判断%d,分型类型%d'%(idx, fenType)
        ...
        for j in range(i+1, lenBiIdx)[0::2]:
            #同向相对高低点
            if (fenType==-1 and tongxiang_price_>chanK['low'][biIdx[j-1]])             or (fenType==1 and tongxiang_price_<chanK['high'][biIdx[j-1]]):
...
def con2Cxianduan(stock, k_data, chanK, frsBiType, biIdx, end_date, cur_ji = 1):
    ...
    def judge_day_bao(chanK, chanKidx, chanKc, xdIdxcn, cur_ji):
        ...
        _start_date = chanK.index[chanKidx] if chanKidx==0        else chanK['enddate'][chanKidx-1]+datetime.timedelta(minutes=1)
...
#             print '开始逆向连接'
            _mid = [lastbi] if (lastBiType == -1 and chanK['low'][lastbi]<=chanKc['low'][firstbic])            or (lastBiType == 1 and chanK['high'][lastbi]>=chanKc['high'][firstbic]) else            [chanKidx for chanKidx in range(len(chanK.index))[biIdx[len(biIdx)-1]:]            if judge_day_bao(chanK, chanKidx, chanKc, firstbic, cur_ji)]
